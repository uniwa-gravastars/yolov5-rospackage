...
class ObjectDetector:
    ...
    def __init__(self):
        ...
        WEIGHTS_PATH = rospy.get_param(
            "~weights_path", os.path.join(ROS_YOLOV5_PATH, "weights", "sciroc2021_ep1.pt")
        )
        ...
        detection_image_topic = rospy.get_param(
            "~detection_image_topic", "/object_detection/image"
        )
        self.image_pub = rospy.Publisher(detection_image_topic, Image, queue_size=detection_queue)

        detection_counter_topic = rospy.get_param(
            "~detection_counter_topic", "/object_detection/counter"
        )
        self.detection_counter_pub = rospy.Publisher(
            detection_counter_topic, String, queue_size=detection_queue
        )

        detection_names_topic = rospy.get_param(
            "~detection_names_topic", "/object_detection/names"
        )
        self.detection_names_pub = rospy.Publisher(
            detection_names_topic, String, queue_size=detection_queue
        )

        detection_objects_topic = rospy.get_param(
            "~detection_objects_topic", "/object_detection/objects"
        )
        self.detection_objects_pub = rospy.Publisher(
            detection_objects_topic, String, queue_size=detection_queue
        )
